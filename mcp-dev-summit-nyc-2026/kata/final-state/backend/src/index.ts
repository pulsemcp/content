...
const slackChannel = process.env.SLACK_CHANNEL || '#linear-clone-notifications';
const slack = slackToken ? new WebClient(slackToken) : null;

if (!slackToken) {
  console.warn('SLACK_BOT_TOKEN not set – Slack notifications disabled');
}
...
app.patch('/api/issues/:id', async (req, res) => {
  try {
    ...
    const current = await pool.query('SELECT * FROM issues WHERE id = $1', [req.params.id]);
    ...
    // Send Slack notification when status changes to "done"
    if (slack && status === 'done' && oldStatus !== 'done') {
      try {
        await slack.chat.postMessage({
          channel: slackChannel,
          text: `*${updatedIssue.identifier}* marked as done: ${updatedIssue.title}`,
        });
        console.log(`Slack notification sent for ${updatedIssue.identifier}`);
      } catch (slackErr) {
  ...
  try {
    ...
    if (parseInt(rows[0].count) === 0) {
      await client.query(`
        ...
        ('SAN-1', 'Sample Issue 1', '', 'backlog', 'p0', NULL),
